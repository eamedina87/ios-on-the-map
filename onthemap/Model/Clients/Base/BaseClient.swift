...
class BaseClient: NSObject {
    ...
    // MARK: GET
    func taskForGETMethod(_ method: String, isForParse:Bool, parameters: [String:AnyObject], completionHandlerForGET: @escaping (_ result: Data?, _ error: NSError?) -> Void) -> URLSessionDataTask {
        
        /* 1. Set the parameters */
        var newParameters = parameters
        if (isForParse){
            newParameters[UDClient.ParameterKeys.Limit] = UDClient.Constants.ParseApiLimit as AnyObject?
        }
    ...
    // MARK: POST
    func taskForPOSTMethod(_ isForParse:Bool = true, parameters: [String:AnyObject], jsonBody: String, completionHandlerForPOST: @escaping (_ result: Data?, _ error: NSError?) -> Void) -> URLSessionDataTask {
    ...
    private func convertDataWithCompletionHandler(_ data: Data, completionHandlerForConvertData: (_ result: AnyObject?, _ error: NSError?) -> Void) {
        ...
        var parsedResult: AnyObject? = nil  //[String: AnyObject]!
        do {
            parsedResult = (try JSONSerialization.jsonObject(with: data, options: .allowFragments) as? AnyObject)!//[String:AnyObject]
        } catch {
